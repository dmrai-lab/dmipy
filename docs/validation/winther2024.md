...
>
> **dmipy-sim, on their own XNH axon meshes: matched** (a few % → ~17 %, tracking each axon's
> morphology; ≈0 for a straight circular axon).
...
!!! warning "Preliminary — publishes with the susceptibility release"
    Per-axon values are Monte-Carlo-noise limited (the metric is a small difference of two
    diffusivities); the population trend and morphology dependence are robust. A converged,
    full-tensor comparison to Winther's per-axon numbers is in progress.
